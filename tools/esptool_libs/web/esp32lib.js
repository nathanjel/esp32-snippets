 ...
 */
import { hexy as _hexy } from 'hexy'; // Dumping hex data utility.
import assert from 'assert'; // Internal assertion.
import md5 from 'md5'; // Hash checksum.
...
const flasher_stub_esp32 = {
  textAddress: '40090000',
  dataAddress: '3ffd2ba4',
  entryPoint: '40090564',
  textData:
...
function SerialData() {
  ...
  function append(data) {
    ...
    if (resolveMoreData != null) {
      // If we have a promise resolution that is waiting for more data ..
      resolveMoreData(); // resolve the promise.
      resolveMoreData = null;
    }
  } // append
  ...
  async function getNextDataPacket() {
    var p = new Promise(async (resolve, reject) => {
      ...
      while (true) {
        // Keep looping until we have a data packet.
        if (DataPacket().containsSLIPPacket(serialDataBuffer)) {
          // Does the buffer now contain a SLIP packet?
          ...
          console.log('Got a new data packet: %s\n%s', newDataPacket.toString(), _hexy(newDataPacket.getData()));
...
function DataPacket() {
  ...
  function getSLIPTrailer(data) {
    ...
    for (i = 1; i < data.length; i++) {
      // Walk through all the bytes in the data supplied.
      if (data[i] == 0xc0) {
        // If we find a slip trailer, return its index.
        return i;
      }
    }
  ...
  function setSLIPPacket(data) {
    ...
    if (data[0] != 0xc0) {
      // if the first byte is not a SLIP marker, then we need to dump
      var begin = getSLIPTrailer(data);
    ...
    for (var i = 1, j = 0; i < end; i++, j++) {
      // Remove escape sequences.
      if (data[i] == 0xdb && data[i + 1] == 0xdc) {
        // 0xdb 0xdc -> 0xc0
        dataPacket[j] = 0xc0;
        i++;
      } else if (data[i] == 0xdb && data[i + 1] == 0xdd) {
        // 0xdb 0xdd -> 0xdb
        dataPacket[j] = 0xdb;
        i++;
      } else {
    ...
    console.log(
      'setSLIPPacket: New data has length: %d, returning unused buffer of size: %d',
      dataPacket.length,
      temp.length
    );
    if (temp.length > 0) {
  ...
  function getSLIPPacket() {
    ...
    temp[i] = 0xc0;
    i++;
    for (j = 0; j < dataPacket.length; j++, i++) {
      // If the current byte is 0xc0 escape to 0xdb 0xdc
      if (dataPacket[j] == 0xc0) {
        temp[i] = 0xdb;
        i++;
        temp[i] = 0xdc;
      } else if (dataPacket[j] == 0xdb) {
        // If the current byte is 0xdb escape to 0xdb 0xdd
        temp[i] = 0xdb;
        i++;
        temp[i] = 0xdd;
      } else {
...
function commandToString(command) {
  switch (command) {
    ...
    case COMMAND_RUN_USER_CODE:
      return 'COMMAND_RUN_USER_CODE';
    case COMMAND_OHAI:
      return 'COMMAND_OHAI';
  }
...
async function buildAndSendRequest(command, data, checksum = 0) {
  console.log(
    'buildAndSendRequest: %s, checksum: 0x%s, dataLength: %d',
...
function ProcessResponse() {
  ...
  function processPacket(inputData) {
    ...
    if (inputData.readUInt8(0) != 0x01) {
      // is this OHAI packet?
      if (inputData.readUInt32LE(0) == 0x4941484f) {
        command = COMMAND_OHAI;
        dataSize = 0;
        value = 0;
        data = null;
        status = 0;
    ...
    errorCode = inputData.readUInt8(inputData.length - 1);
    clearTimeout(timeoutId);
  } // processPacket
...
async function uploadFlasher() {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    // var flasherDataRaw = readFileSync('esptool/flasher_stub/build/flasher_stub.json', 'utf8');
    var flasherData = flasher_stub_esp32;
...
function writeReg(address, value) {
  ...
  console.log('+-----------+');
  console.log(
    '>> Writing register:  Command: 0x%s, Address: 0x%s, value: 0x%s',
    COMMAND_WRITE_REG.toString(16),
    address.toString(16),
    value.toString(16)
  );
  var p = new Promise(async function (resolve, reject) {
...
function spiSetParams(size) {
  ...
  console.log('+----------------+');
  console.log('>> SPI Set Params:  Command: 0x%s, size:%d', COMMAND_SPI_SET_PARAMS.toString(16), size);
  var p = new Promise(async function (resolve, reject) {
    await flush();
...
function readReg(address) {
  console.log(
    '>> Reading register:  Command: 0x%s, Address: 0x%s',
    COMMAND_READ_REG.toString(16),
    address.toString(16)
  );
  var p = new Promise(async function (resolve, reject) {
    await flush();
...
function spiFlashMD5(address, size) {
  ...
  console.log('+---------------+');
  console.log(
    '>> spiFlashMD5:  Command: 0x%s, Address: 0x%s, Size: %d',
    COMMAND_SPI_FLASH_MD5.toString(16),
    address.toString(16),
    size
  );
  var p = new Promise(async function (resolve, reject) {
...
function reboot() {
  console.log('>> reboot to user code');
  return new Promise((resolve, reject) => {
    setFlags({ dtr: false, rts: false })
      .then(() => {
        return delay(150);
      })
      .then(() => {
        return setFlags({ dtr: false, rts: true });
      })
      .then(() => {
        return delay(150);
      })
 ...
 */
async function flashFile(address, fileData) {
  console.log('Flashing file %s to address 0x%s', fileName, address.toString(16));
  console.log('Data size is %d, md5: %s', fileData.length, md5(fileData));
...
function setPort(nport) {
  port = nport;
  port.on("data", (data) => {
    console.log("ESP32LIB backend: Port received data");
    serialData.append(data);  // Append the new data into the serial buffer.
  });
}

function callProgrammer() {
  port.open(async (err)=>{
      if (err) {
      ...
      */
      ...
      console.log("booting to user code...");
      await reboot();
...
export {
  ...
  COMMAND_WRITE_REG,
  COMMAND_READ_REG,
  COMMAND_SPI_SET_PARAMS,
  COMMAND_SPI_ATTACH,
  COMMAND_CHANGE_BAUDRATE,
  COMMAND_FLASH_DEFL_BEGIN,
  COMMAND_FLASH_DEFL_DATA,
  COMMAND_FLASH_DEFL_END,
  COMMAND_SPI_FLASH_MD5,
  COMMAND_ERASE_FLASH,
  COMMAND_ERASE_REGION,
  COMMAND_READ_FLASH,
  COMMAND_RUN_USER_CODE,
  EFUSE_REG_BASE,
  flasher_stub_esp32,
  SerialData,
  DataPacket,
